...
|---|---|---|---|
| `--llm-live` | `llm_live` | `OPENROUTER_API_KEY` | Enable tests that call real LLMs |
...
Add `OPENROUTER_API_KEY` as a repository secret, then use it in your workflow:
...
- **Two-layer gating**: LLM tests require both `--llm-live` flag AND `OPENROUTER_API_KEY` env var. Network tests require `--live-network`. Missing either skips with a clear reason.
...
- With `--llm-live` but skipped: Check `set -a; source .env; set +a` was run and `OPENROUTER_API_KEY` is set (`echo $OPENROUTER_API_KEY`).
