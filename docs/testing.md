...
└── live/                    # live LLM integration tests
    ...
    ├── test_cli_live.py     # L5 — CLI lifecycle (registry, git)
    └── test_plugins.py      # L6 — plugin tests (clone + run internal tests)
```
...
# L5 — registry + git, no LLM
uv run pytest tests/live/test_cli_live.py --live-network -v

# L6 — clone and test each official plugin
...
Functional and live tests that install wrappers/connectors download binaries and Python packages on first run. A single Docker named volume (`dep-cache`) caches `~/.cache/` across runs, covering all dependency managers:
