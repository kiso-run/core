...
## Prossime cose da fare (candidati milestone)

### Qualità CLI
- ~~**M76**: `kiso user edit`~~ ✅ `cli/user.py` — `_user_edit` con guard last-admin, `_parse_skills`, argparse choices
- ~~**M77**: `kiso user list --json`~~ ✅ `cli/user.py` — `if getattr(args, "json", False): print(json.dumps(users))`

### Robustezza runtime
- ~~**M78**: `--no-reload`~~ ✅ `cli/user.py` — flag `--no-reload` su add/edit/remove/alias; `_maybe_reload(args)` helper
- ~~**M79**: loop error recovery~~ ✅ `kiso/worker/loop.py` — `isinstance(learn_raw, list/str)` + warning se stringa

### Test / copertura
- ~~**M80**: integration test per `cli/__init__.py`~~ ✅ `tests/test_cli_integration.py` — 14 test end-to-end (list, list --json, add, edit, remove, alias) che chiamano `main()` via `sys.argv` patchato; unici mock: `require_admin`, `CONFIG_PATH_DEFAULT`, `_call_reload`
- ~~**M81**: controlla coverage di `kiso instance *`~~ ✅ `cli/instance.py` non esiste; coverage audit ha identificato `cli/stats.py` a 0% → aggiunto `tests/test_cli_stats.py` con 23 test (_fmt_k, _fmt_cost, print_stats, run_stats_command); ora al 100%

### Planner / brain
- ~~**M82**: planner test per workflow "ask-then-add"~~ ✅ `TestM82PlannerAskThenAdd` (3 test): (1) `build_planner_messages` inietta `## Caller Role\nuser`; (2) `run_planner` accetta un msg-only plan; (3) cattura messaggi LLM e verifica che caller role e richiesta siano visibili insieme
- ~~**M83**: reviewer schema enforcement~~ ✅ aggiunto `jsonschema>=4` a dev deps; `TestM83PlanSchema` (11 test: campi required, additionalProperties, enum task type, extend_replan type) + `TestM83ReviewSchema` (9 test: status enum, retry_hint, learn maxItems/items type, extra field)

---

## Code review — bug, performance, maintainability (2026-03-02)

Risultato di una review completa della codebase. Ordinati per priorità: prima i crash certi, poi i bug difensivi, poi performance, poi manutenibilità.

### Bug — crash certi o quasi

- **M84**: `lifespan()` shutdown — `main.py`: il primo loop `for session, entry in _workers.items()` itera senza `list()` mentre i cleanup callback possono rimuovere elementi dal dict → `RuntimeError: dictionary changed size during iteration`; fix: `list(_workers.items())`

- **M85**: `_retry_llm_with_validation()` — `brain.py` ~78: `json.JSONDecodeError` lancia immediatamente invece di appendere a `last_errors` e riprovare come fanno gli errori di validazione; il modello ha buone probabilità di produrre JSON valido al retry successivo

- **M86**: difese mancanti in loop/brain — due fix in un colpo:
  - `worker/loop.py` handler exec/skill/search: `review["reason"]` con accesso diretto → `.get("reason")` per resistere a risposte LLM fuori schema
  - `brain.py` `run_fact_consolidation` ~939: `float(item.get("confidence", 1.0))` senza `try/except ValueError` → wrap con fallback a `1.0`

### Bug — robustezza config

- **M87**: validazione settings al load — `config.py`: `settings.update(settings_raw)` accetta qualsiasi tipo senza check; `int(config.settings["max_plan_tasks"])` in `brain.py` crasha a runtime invece che al config load; aggiungere validazione dei tipi attesi sui campi critici (max_plan_tasks, max_validation_retries, context_messages, ecc.) in `load_config()`; inoltre i blocchi `except` in `_build_config` chiamano `on_error()` senza garantire che l'esecuzione si fermi dopo

### Performance

- **M88**: cache `discover_skills()` — `brain.py` ~374: filesystem scan + TOML parse su ogni chiamata al planner; aggiungere cache con TTL (es. 30s) o invalidazione esplicita via `invalidate_skills_cache()`; stessa funzione serve `installed_names` per `validate_plan()` — il risultato può essere riusato

- **M89**: track used fact IDs — `brain.py` `build_planner_messages` + `worker/loop.py` ~1147: al completamento del plan vengono aggiornati i `use_count` di **tutte** le facts (`get_facts(all)` → `update_fact_usage(tutti gli id)`); restituire invece gli id delle facts effettivamente iniettate nel contesto e aggiornare solo quelle

- **M90**: batch INSERT nella fact consolidation — `worker/loop.py` ~249: dopo la consolidation, `save_fact()` è chiamato in loop con commit individuale per ogni riga (N transazioni SQLite); raccogliere tutti i valori e fare un singolo INSERT multi-row + commit

- **M91**: dict lookup per installed skills — `worker/loop.py` ~627: `next(s for s in ctx.installed_skills if s["name"] == skill_name)` è O(n) su lista; costruire `{s["name"]: s for s in installed}` una volta sola quando le skills vengono scoperte e passarlo nel contesto

### Manutenibilità

- **M92**: costanti per tipi task e status review — `brain.py`: `"exec"`, `"msg"`, `"skill"`, `"search"`, `"replan"` e `"ok"`, `"replan"` compaiono come stringhe magiche in schema, `validate_plan`, `validate_review`, `worker/loop.py`; definire `TASK_TYPES`, `REVIEW_STATUSES`, `CURATOR_VERDICTS` come tuple/frozenset a livello di modulo

- **M93**: shared HTTP auth helper CLI — `cli/user.py` e `cli/stats.py` duplicano il pattern `load_config()` → token → `httpx.get(..., headers={"Authorization": "Bearer ..."})` ; estrarre in `cli/_http.py` una funzione `cli_get(args, path, params=None)` che gestisce auth, ConnectError e HTTPStatusError uniformemente

- **M94**: estrarre `_startup_recovery()` da `lifespan()` — `main.py`: il blocco di recovery al startup (~righe 155-194, ~40 righe) è logica autonoma che re-accoda i task interrotti; estrarre in funzione separata riduce `lifespan()` da ~87 a ~50 righe e la rende testabile

- **M95**: `build_*_messages()` — `brain.py`: le 5 funzioni (`build_planner_messages`, `build_reviewer_messages`, `build_messenger_messages`, `build_exec_translator_messages`, `build_paraphraser_messages`) seguono tutte lo stesso schema (load prompt → lista context parts → `[system, user]`); valutare se estrarre un helper `_build_messages(role, parts)` per il pattern comune

- **M96**: split `_render_plan_status()` — `cli/__init__.py`: funzione da ~209 righe che gestisce inline la rendering di tutti i tipi di task (msg, exec, skill, search, replan); estrarre un renderer per tipo (`_render_msg_task`, `_render_exec_task`, ecc.) per rendere ogni tipo modificabile e testabile indipendentemente
