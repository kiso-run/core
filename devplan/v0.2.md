...
- [ ] **84a** — `lifespan()` shutdown (`main.py`): first loop `for session, entry in _workers.items()` iterates without `list()` while cleanup callbacks can remove entries from the dict → `RuntimeError: dictionary changed size during iteration`; fix: `list(_workers.items())`

- [ ] **84b** — `_retry_llm_with_validation()` (`brain.py` ~78): `json.JSONDecodeError` raises immediately instead of appending to `last_errors` and retrying like validation errors do; the model often produces valid JSON on a retry

- [ ] **84c** — `review["reason"]` direct access (`worker/loop.py`, exec/skill/search handlers): should be `.get("reason")` to survive LLM responses that omit the field despite it being in the schema

- [ ] **84d** — `float(item.get("confidence", 1.0))` (`brain.py`, `run_fact_consolidation` ~939): no `try/except ValueError`; crashes if the curator LLM returns `"confidence": "high"`; wrap with fallback to `1.0`

- [ ] **84e** — config type validation at load time (`config.py`): `settings.update(settings_raw)` accepts any type without checking; `int(config.settings["max_plan_tasks"])` in `brain.py` crashes at runtime instead of at load; validate expected types for critical keys (`max_plan_tasks`, `max_validation_retries`, `context_messages`, etc.) in `load_config()`; also: `except` blocks in `_build_config` call `on_error()` without guaranteeing execution stops after

- [ ] **84f** — `_apply_curator_result()` unsafe dict access (`worker/loop.py` ~1058): direct `ev["learning_id"]`, `ev["verdict"]`, `ev["fact"]`, `ev["question"]` accesses will `KeyError` if the curator LLM omits any field; switch to `.get()` with safe defaults; also: exceptions raised inside `_apply_curator_result()` are not caught by the surrounding curator `try/except`, so a malformed curator response can crash the entire consolidation loop

- [ ] **84g** — `cli/__init__.py` unsafe JSON key access: `data["message_id"]` at lines 423 and 487 (send/msg response handling) and `info["session"]`, `info["message_count"]` at lines 984-985 (session info display) crash with `KeyError` on unexpected server responses; switch to `.get()` with graceful fallbacks

- [ ] **84h** — `cli/user.py _read_raw()` unhandled `FileNotFoundError` (~line 38): if `config.toml` does not exist the function propagates `FileNotFoundError` instead of returning an empty dict; fix: catch `FileNotFoundError` and return `{}`

- [ ] **84i** — prompt contradictions and validation gap:
