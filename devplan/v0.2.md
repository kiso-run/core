...
**Completed**: M44–M72, M84, M85, M86, M87, M88, M89, M90, M91, M92, M93, M94, M95, M96, M97, M98, M99

**Planned**: M102, M103, M104, M105

**Note**: M102, M103, M104, M105 complete.
...
- [ ] `kiso/worker/loop.py`: add `_validate_plan_search_tasks(plan)` check after JSON validation passes
- [ ] Pattern: reject only when the search is clearly about kiso plugin discovery, not general web search
- [ ] On rejection: return validation error with hint, so the planner retries with guidance
- [ ] Test: plan with search "find browser skill in kiso registry" → rejected; plan with search "latest python release" → accepted
...
- [ ] `kiso/worker/loop.py` (or wherever `call_llm` is invoked for the translator): pass `max_tokens=500`
- [ ] `kiso/llm.py`: if `call_llm` doesn't already support `max_tokens`, add it and pass to the API request
- [ ] Test: translator call with `max_tokens=500` in test config
...
- [ ] `kiso/worker/loop.py`: add `_repair_json(raw: str) -> str` before `json.loads()`
- [ ] Strip markdown code fences, trailing commas, leading/trailing whitespace
- [ ] On parse failure: include error message and position in the retry context
- [ ] Test: JSON with trailing comma → repaired; JSON in markdown fence → extracted; valid JSON → unchanged
