...
### Qualità CLI
- **M76**: `kiso user edit` — modifica in-place di role/skills/aliases su utente esistente (evita remove+add)
- **M77**: `kiso user list --json` — output machine-readable per scripting e CI

### Robustezza runtime
- **M78**: timeout/retry nel CLI — `_call_reload` ora fa exit(1) se il server non è raggiungibile; aggiungere flag `--no-reload` per operare offline (utile durante setup iniziale)
- **M79**: loop error recovery — se `learn` arriva come stringa invece che array (risposta LLM malformata), non crashare con `TypeError: can only join an iterable`; aggiungere fallback `[learn]` + log warning
