...
**Completed**: M44–M72, M84, M85, M86, M87, M88, M89, M90

**Planned**: M91 (worker refactor)
...
- [ ] **91a** — duplicated error recovery in `_process_message` (`loop.py` ~1477-1504): `asyncio.TimeoutError`, `PlanError`, and `ParaphraserError` handlers each execute the same six DB operations (create_plan → create_task → update_task → update_plan_status → save_message → deliver_webhook); extract `async def _handle_plan_error(db, config, session, msg_id, error_text)` and delegate from all three handlers

- [ ] **91b** — `_run_planning_loop` is 235 lines with six distinct termination cases (`loop.py` ~1167-1401): success, failure, cancel, replan, extend-replan, and max-depth each perform a separate set of DB operations and message creation; extract `async def _handle_plan_success(...)`, `async def _handle_plan_failure(...)`, `async def _handle_plan_cancellation(...)` to reduce the main loop to a readable dispatcher

- [ ] **91c** — `ctx.plan_outputs` mutated in-place by handlers (`loop.py` ~521, 708, 812): handlers append to `ctx.plan_outputs` directly rather than returning their output; this makes data flow implicit and the retry overwrite logic (exec handler lines 809-812) relies on index knowledge; add `plan_output: dict | None = None` to `_TaskHandlerResult` and have `_execute_plan` collect results, removing the mutation from handlers
