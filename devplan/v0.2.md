...
- [ ] `cli/`: implement `user` subcommand with the four commands above
- [ ] `kiso-host.sh`: the catch-all section already proxies everything via `docker exec` — no changes needed
- [ ] `tests/test_cli_user.py`: unit tests for each command — add ok, add missing role, add invalid username, add existing user, remove ok, remove last admin, alias add/update/remove, list
...
- [ ] `kiso/roles/planner.md`: add to "Kiso management commands" section:
  ```
  ...
  ```
- [ ] `kiso/roles/planner.md`: rule — if `Caller Role` is `user`, never generate `kiso user` tasks; respond with a msg explaining the admin requirement
- [ ] `kiso/roles/planner.md`: rule — for add-user requests: if role not specified, emit a msg task asking for role (and skills if user) before proceeding; if running connectors are present in System Environment, ask for aliases for each in the same msg task
- [ ] `kiso/roles/planner.md`: rule — `kiso user add` with `--role admin` does not require `--skills`; with `--role user` `--skills` is required
