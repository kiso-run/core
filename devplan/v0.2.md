...
### Planner / brain
- **M82**: planner test per workflow "ask-then-add" — verifica che se `Caller Role=user` e il planner riceve una richiesta di `kiso user add`, la risposta sia un `msg` task e non un `exec`
- **M83**: reviewer schema enforcement — aggiungere validazione JSON-Schema lato test per `PLAN_SCHEMA` e `REVIEW_SCHEMA` (oggi solo i campi singoli sono testati, non lo schema completo)
