...
**Completed**: M44–M72, M84, M85, M86, M87, M88, M89

**Planned**: M90 (security), M91 (worker refactor)
...
- [ ] **90a** — rate limiting absent on all endpoints (`kiso/main.py`): `/msg` POST, `/admin/reload-config`, `/admin/reload-env`, `/pub/`, `/sessions/{session}/cancel` have no per-user or per-IP request limits; add middleware-level rate limiting (e.g. `slowapi` or a custom `asyncio`-based token-bucket); suggested limits: 20 msg/min per user, 5 admin calls/min, 60 pub requests/min per IP

- [ ] **90b** — `reload-env` sets arbitrary env vars without validation (`main.py` ~537): `os.environ[key] = value` is called for every key in `.kiso/.env` with no whitelist; a malformed or malicious `.env` file can overwrite `PYTHONPATH`, `LD_PRELOAD`, API keys, or inject multi-line values; add an allowlist of accepted key prefixes (e.g. `KISO_`, `OPENAI_`, `ANTHROPIC_`, `OLLAMA_`), validate values (no newlines, max 1024 chars), and log all applied changes

- [ ] **90c** — `/status/{session}` missing session ownership check (`main.py` ~358): any authenticated user can query the status of any session by guessing or knowing the session name; non-admin users should only be able to read their own sessions; add a check that `session` belongs to the authenticated user or that the user has admin role, returning 403 on violation
