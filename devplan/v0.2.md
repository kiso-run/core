...
- [x] **89c** — `get_facts()` without LIMIT in `build_messenger_messages` (`brain.py`): added `limit: int | None = None` parameter to `get_facts()` (applies `LIMIT ?` when set); capped the messenger call at `limit=50` — the consolidation loop in `loop.py` intentionally keeps no cap to compare against `knowledge_max_facts`

- [x] **89d** — blocking `write_text()` in worker async context (`worker/utils.py`): `_write_plan_outputs()` made `async`; `write_text` offloaded via `asyncio.get_running_loop().run_in_executor(None, path.write_text, content)`; all 3 call sites in `loop.py` updated with `await`; `TestWritePlanOutputs` and `TestCleanupPlanOutputs` tests updated to `async`
