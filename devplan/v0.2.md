...
- **85b** — track used fact IDs (`brain.py` `build_planner_messages` + `worker/loop.py` ~1147): on plan completion, `get_facts(all)` + `update_fact_usage(all IDs)` updates every fact in the DB; return the IDs of facts actually injected into context and update only those
...
- **86b** — shared CLI HTTP helper (`cli/user.py` + `cli/stats.py`): both duplicate the pattern `load_config()` → token → `httpx.get(..., headers={"Authorization": "Bearer ..."})` with identical error handling; extract to `cli/_http.py` as `cli_get(args, path, params=None)` covering auth, `ConnectError`, and `HTTPStatusError` uniformly
