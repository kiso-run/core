...
**Completed**: M44–M72, M84, M85, M86, M87

**Planned**: M88 (tech debt), M89 (performance), M90 (security), M91 (worker refactor)
...
- [ ] **88a** — unused `tasks: list` parameter in all task handlers (`kiso/worker/loop.py`): `_handle_msg_task`, `_handle_exec_task`, `_handle_skill_task`, `_handle_search_task` all accept `tasks` but never read it; the parameter suggests handlers have access to sibling tasks but they don't use it, creating a misleading contract; remove from all handler signatures and the dispatch call site

- [ ] **88b** — magic substatus string literals (`kiso/worker/loop.py`): `"translating"`, `"executing"`, `"reviewing"`, `"composing"` appear scattered across handlers and `_fast_path_chat`; extract as module-level constants `_SUBSTATUS_TRANSLATING`, `_SUBSTATUS_EXECUTING`, `_SUBSTATUS_REVIEWING`, `_SUBSTATUS_COMPOSING` analogous to the `TASK_TYPE_*` and `REVIEW_STATUS_*` constants added in M86a
