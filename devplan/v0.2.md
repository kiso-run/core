...
- [ ] **85a** — cache `discover_skills()` (`brain.py` ~374): full filesystem scan + TOML parse on every planner call; add a TTL cache (e.g. 30 s) or explicit invalidation via `invalidate_skills_cache()`; the result also feeds `validate_plan()` and can be reused within the same request

- [ ] **85b** — track used fact IDs (`brain.py` `build_planner_messages` + `worker/loop.py` ~1147, ~1397): on plan completion and on fast-path chat, `get_facts(all)` + `update_fact_usage(all IDs)` updates every fact in the DB; return the IDs of facts actually injected into context and update only those

- [ ] **85c** — batch INSERT in fact consolidation (`worker/loop.py` ~249): `save_fact()` is called in a loop with one commit per row (N SQLite transactions); collect all values and execute a single multi-row INSERT + commit

- [ ] **85d** — dict lookup for installed skills (`worker/loop.py` ~627): `next(s for s in ctx.installed_skills if s["name"] == skill_name)` is O(n) per skill task; build `{s["name"]: s for s in installed}` once when skills are discovered and store it in context

- [ ] **85e** — missing DB index on `facts(category, session)` (`store.py`): `get_facts()` filters with `WHERE category != 'user' OR session = ?` but there is no index covering these columns; every planner, reviewer, and curator call does a full table scan; fix: add `CREATE INDEX IF NOT EXISTS idx_facts_cat_sess ON facts(category, session)` to the schema init in `create_tables()`
