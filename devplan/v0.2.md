...
- [ ] **86a** — named constants for task types and review statuses (`brain.py`): `"exec"`, `"msg"`, `"skill"`, `"search"`, `"replan"` and `"ok"`, `"replan"` appear as magic strings in the schema, `validate_plan`, `validate_review`, and `worker/loop.py`; define `TASK_TYPES`, `REVIEW_STATUSES`, `CURATOR_VERDICTS` as module-level constants
...
- [ ] **86c** — extract `_startup_recovery()` from `lifespan()` (`main.py`): the startup recovery block (~lines 155-194, ~40 lines) is self-contained logic that re-enqueues interrupted tasks; extracting it reduces `lifespan()` from ~87 to ~50 lines and makes it independently testable
...
- [ ] **86e** — split `_render_plan_status()` (`cli/__init__.py`): ~209-line function that inlines rendering logic for all task types (msg, exec, skill, search, replan); extract per-type renderers (`_render_msg_task`, `_render_exec_task`, etc.) so each type can be modified and tested independently

- [ ] **86f** — deduplicate `_skill_list()` / `_connector_list()` (`cli/skill.py:80-109`, `cli/connector.py:207-235`): the two functions are ~95% identical (~50 lines each), differing only in item type label and discover function; extract `_list_plugins(discover_fn, item_type: str)` in `cli/plugin_ops.py` and delegate from both

- [ ] **86g** — test coverage gaps:
...
- [ ] **86h** — `cli/connector.py` unused import: `import logging` and `log = logging.getLogger(__name__)` are defined but never referenced; remove both

- [ ] **86i** — `cli/stats.py` redundant `getattr` defaults: `since = getattr(args, "since", 30)` etc. repeat the same defaults already declared in argparse (`add_argument("--since", default=30, ...)`); replace with direct attribute access `args.since`, `args.by`, `args.session`

- [ ] **86j** — unnamed magic literals:
...
- [ ] **86k** — verbose per-item learning log (`worker/loop.py` ~463): `log.info("Learning saved: %s", item[:100])` fires for every fact item during consolidation; for sessions with many facts this floods the log; demote to `log.debug()`

- [ ] **86l** — inconsistent error output destination in CLI: `cli/__init__.py` sends some errors to `sys.stderr` while all other CLI modules (`stats.py`, `env.py`, `session.py`, `user.py`) print errors to stdout; standardize on `sys.stderr` for error messages across all CLI modules
