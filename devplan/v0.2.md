...
- [ ] `pyproject.toml` + `uv.lock`: add `tomli-w>=1.0` as a runtime dependency
- [ ] `kiso/main.py`: new `POST /admin/reload-config` endpoint — verifies `role == "admin"`, calls `reload_config()`, updates `app.state.config`, calls `invalidate_prompt_cache()` (bot_name or settings may have changed); returns `{"reloaded": True}`; on `ConfigError` returns 400 with the error message
- [ ] `docs/api.md`: document the new endpoint
- [ ] `tests/test_main.py`: 4 tests — reload ok (admin), 403 (non-admin), 400 (invalid config), cache invalidation called
