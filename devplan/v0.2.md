...
- [ ] **86d** — common pattern in `build_*_messages()` (`brain.py`): eight functions (`build_planner_messages`, `build_reviewer_messages`, `build_messenger_messages`, `build_exec_translator_messages`, `build_paraphraser_messages`, `build_classifier_messages`, `build_curator_messages`, `build_searcher_messages`) all follow the same structure (load prompt → build context parts list → return `[system, user]`); evaluate extracting a `_build_messages(role, parts)` helper for the shared skeleton
