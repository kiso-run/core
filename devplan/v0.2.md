...
**Completed**: M44–M72, M84, M85, M86, M87, M88
...
- [ ] **89a** — missing index on `messages(user)` (`store.py`): `get_sessions_for_user()` filters with `WHERE m.user = ?` on a JOIN but no index covers `messages(user)`; every `/sessions` request does a full table scan; add `CREATE INDEX IF NOT EXISTS idx_messages_user ON messages(user)` to `create_tables()`

- [ ] **89b** — N+1 queries in `create_session` / `upsert_session` (`store.py`): both functions call `get_session()` twice (check-then-insert-then-re-fetch); replace with `INSERT OR IGNORE` + single final SELECT, or use `RETURNING` (SQLite ≥ 3.35) to eliminate the second query

- [ ] **89c** — `get_facts()` without LIMIT in `build_messenger_messages` (`brain.py` ~793): the messenger receives all facts with no cap; as facts accumulate the LLM context grows unbounded (50 facts ≈ 2 KB, 500 facts ≈ 20 KB); add a `limit` parameter to `get_facts()` and cap the messenger call at a reasonable default (e.g. 50), consistent with the existing `search_facts()` cap

- [ ] **89d** — blocking `write_text()` in worker async context (`worker/utils.py` ~100): `_write_plan_outputs()` calls `Path.write_text()` synchronously from inside the async worker loop; for large plan output JSON this blocks the event loop; wrap with `asyncio.get_event_loop().run_in_executor(None, ...)`

- [ ] **89e** — blocking `subprocess.run()` in `_ensure_sandbox_user()` (`worker/utils.py` ~125): `useradd` is called with `subprocess.run()` (synchronous) inside the async task execution path; on user-role sessions this blocks the entire event loop for the duration of the system call (~50-200 ms per task); replace with `await asyncio.get_event_loop().run_in_executor(None, _ensure_sandbox_user_sync, session)`
