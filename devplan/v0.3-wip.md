...
- [ ] `cli/__init__.py`: `cmd_version()` (or the `elif args.command == "version"` block) — when `--stats` is absent, call `count_loc(root)` and print `f"kiso {__version__}  ({_fmt(total)} loc)"` instead of the bare version string
- [ ] `cli/__init__.py`: extract `_fmt` helper (currently duplicated inside `_print_version_stats`) to module level so the plain path can reuse it
- [ ] `tests/test_cli.py`: `test_version_plain_shows_loc` — capture stdout of `cmd_version(args)` with no `--stats`; assert output matches `kiso X.Y.Z  (N loc)` pattern and `N > 0`
- [ ] `tests/test_cli.py`: `test_version_stats_unchanged` — existing `--stats` output still shows per-area rows and total line
