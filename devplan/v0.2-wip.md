...
Nuovo modulo standalone (no I/O, no side-effects — solo pura logica):

- `read_audit_entries(audit_dir: Path, since: datetime | None = None) -> list[dict]`
  — legge tutti i `*.jsonl` in `audit_dir/`, filtra `type == "llm"`, filtra per `timestamp >= since`; skip silenzioso su righe malformate e file illeggibili

- `aggregate(entries: list[dict], by: str) -> list[dict]`
  — `by` in `("model", "session", "role")`; ritorna lista di `{key, calls, errors, input_tokens, output_tokens}` ordinata per `input_tokens + output_tokens` desc

- `MODEL_PRICES: dict[str, tuple[float, float]]`
  — price table statica `{"model-substring": (in_per_mtok, out_per_mtok)}`; lookup per substring match (es. `"gemini-flash"` → 0.075, 0.30); `None` se nessun match

- `estimate_cost(row: dict) -> float | None`
  — calcola `(in * price_in + out * price_out) / 1_000_000`; None se modello sconosciuto

**Test**: `tests/test_stats.py` — 8 test:
  - `read_audit_entries`: dir vuota → `[]`; righe malformate saltate; filtro `type != "llm"` esclude; filtro `since` funziona
  - `aggregate(by="model")`: raggruppa correttamente; ordine per volume; errors contati separati
  - `estimate_cost`: modello noto → float; modello sconosciuto → None; totale zero → 0.0

### 54b. API endpoint `GET /admin/stats`

In `kiso/main.py` (admin-only, come `GET /admin/reload-env`):

- `GET /admin/stats?since=30&session=NAME&by=model`
  — chiama `read_audit_entries(KISO_DIR / "audit", since=...)` + `aggregate(entries, by=...)`
  — se `session` passato: filtra entries prima di aggregare
  — ritorna `{"by": "model", "since_days": 30, "rows": [...], "total": {...}}`

**Test**: `tests/test_stats.py` → 4 test endpoint (mock audit dir, verifica risposta JSON, authn admin richiesta, filtro sessione)

### 54c. CLI `kiso stats`

In `cli/__init__.py`:

- Nuovo subcommand `stats` con flags:
  - `--since N` — ultimi N giorni (default: 30)
  - `--session NAME` — filtra per sessione
  - `--by model|session|role` — dimensione di aggregazione (default: model)

- Chiama `GET /admin/stats?since=N&session=NAME&by=...` (come gli altri comandi che usano l'API)

- Output (plain, senza rich; colonne allineate, separatore migliaia a spazio):
  ...
  Colonna `est. cost` omessa se tutte le righe hanno costo `None`.

**Test**: `tests/test_cli.py` — 3 test (subcommand registrato; `--since`, `--session`, `--by` flag presenti; output formattato su mock JSON)

### 54d. Wrapper `kiso stats [--all]` in `kiso-host.sh`

- `kiso stats [flags]` — risolve istanza implicita o `--instance NAME`, poi `docker exec kiso-$INST uv run kiso stats [flags]`
- `kiso stats --all [--since N]` — loop su tutte le istanze in `instances.json`; per ognuna:
  - header `── jarvis ──`
  - `docker exec kiso-jarvis uv run kiso stats --since N 2>/dev/null || echo "  (instance not running)"`
  - totale aggregato finale (sommato a livello di wrapper tramite Python one-liner su stdout)

**Test**: `tests/bash/test_host_stats.bats` — 4 test:
  - istanza singola → `docker exec` chiamato; output contiene "Token usage"
  - `--all` con 2 istanze mock → output contiene entrambi i nomi
  - istanza non in esecuzione → messaggio "(instance not running)", exit 0
  - `--instance` esplicita → usa quella
...
| Livello | Stato | Gap |
|---------|-------|-----|
| Shell bash | ✅ `completions/kiso.bash` (148 righe) | Manca `stats`; `_kiso_sessions()` solo single-instance; `explore SESSION` non completato |
| Shell zsh | ✅ `completions/kiso.zsh` (205 righe) | Stessi gap del bash |
| REPL readline | ✅ 7 slash commands | Manca `/stats`; nessuna completion contestuale |
| CLI Python argparse | ✅ subparser | Manca subcommand `completion`; manca `stats` |
| Wrapper kiso-host.sh | ❌ nessun `completion` handler | Da aggiungere |

### 55a. CLI `kiso completion bash|zsh` + handler wrapper

In `cli/__init__.py`:
- Nuovo subcommand `completion` con argomento posizionale `shell` (`bash` o `zsh`)
- Legge il file da `completions/kiso.{shell}` via `importlib.resources` (file incluso in `pyproject.toml` come `package-data`)
- Stampa su stdout → user: `source <(kiso completion bash)`

In `kiso-host.sh`:
- Aggiunge branch `completion)` nel dispatch principale
- `kiso completion bash` → `uv run kiso completion bash` (delega al CLI Python)
- Aggiunge alla help string

**Test**:
- `tests/test_cli.py`: `kiso completion bash` → output contiene `_kiso` e `complete -F`; `kiso completion zsh` → contiene `#compdef kiso`
- `tests/bash/test_host_completion.bats`: 2 test (bash output, zsh output)

### 55b. Completions aggiornate — `stats` + multi-instance sessions + `explore`

In `completions/kiso.bash` e `completions/kiso.zsh`:

**Aggiunge `stats`**:
- Comandi top-level: aggiunge `stats`
- Branch `stats)`: flags `--since --session --by --all`; `--session` chiama `_kiso_sessions()`

**Fix `_kiso_sessions()` per multi-instance**:
- Rileva `--instance NAME` o `-i NAME` nel command line corrente (`${words[@]}`)
- Se trovato → usa quell'istanza per la query SQLite
- Se non trovato e una sola istanza → usa quella implicita
- Se non trovato e più istanze → nessuna completion (silenzioso)

**`kiso instance explore SESSION`**:
- Branch `explore)` in `instance` sub-subcommands → chiama `_kiso_sessions()`

**`kiso --session`** (flag globale):
- Nella sezione top-level flags → `_kiso_sessions()` come completion per il valore di `--session`

### 55c. REPL `/stats` slash command

In `cli/__init__.py`:

- Aggiunge `/stats` a `_SLASH_COMMANDS`
- Handler in `_interactive_loop`:
  - Chiama `GET /admin/stats?since=7&session={SESSION}&by=model` (7 giorni, sessione corrente)
  - Stampa tabella compatta (stessa formattazione di `kiso stats` ma senza header ripetitivi)
  - Su errore di rete → `[stats unavailable]`

**Test**: `tests/test_cli.py` — 2 test (`/stats` presente nei completions; handler chiama endpoint corretto e formatta output)
