...
- [ ] `kiso-host.sh`: controllo `[ -d "$KISO_DIR/sessions/$SESSION" ]` prima di `docker exec`; path `KISO_DIR=~/.kiso/instances/$INSTANCE`
- [ ] `tests/bash/test_host_instance_explore.bats`: aggiornato (già pianificato in 51a)
...
- [ ] `kiso-host.sh`: aggiunto prompt interattivo nel branch `remove` quando `--yes` non presente
- [ ] `tests/bash/test_host_instance_remove.bats`: aggiornato (già pianificato in 51a)
...
- [ ] `kiso-host.sh`: post-run health check con retry; rollback `instances.json` su failure
- [ ] `tests/bash/test_host_instance_commands.bats`: test `create` fallisce → instances.json non contiene l'entry
...
- [ ] `kiso/_version.py`: nuova funzione `count_loc(root: Path) -> dict[str, int]` — per ogni `.py` in `root` (ricorsivo), conta righe non-vuote e non-commento (strip + `not line.startswith("#")` + `len(line) > 0`). Restituisce `{"core": N, "cli": N, "tests": N, "total": N}`.
- [ ] `cli/__init__.py`: `version` subcommand — se `--stats` → chiama `count_loc`, formatta output con `rich` (o plain se `caps.color=False`); se no `--stats` → comportamento attuale (stampa solo versione)
- [ ] `tests/test_version.py`: `TestLocCounter` — 5 test: file vuoto = 0, solo commenti = 0, misto = conteggio corretto, directory inesistente = 0, totale = somma delle aree
