...
**Nota**: la cache è stata implementata e poi rimossa (commit "M61b: rimuovi cache discover_skills — scan ad ogni chiamata"). Il scan è abbastanza rapido (parse TOML di pochi file) da non giustificare la complessità della cache. `discover_skills()` riscansiona ad ogni chiamata — nessuna cache.

- [x] Implementata e poi rimossa: nessun codice di caching nel sorgente corrente
- [x] `tests/test_skills.py`: `TestDiscoverSkillsNoCache` — verifica che le skill appena installate siano visibili immediatamente senza invalidare nulla
