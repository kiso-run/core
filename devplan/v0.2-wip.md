...
- [ ] 50b. `kiso-host.sh`: riscrittura — multi-instance, port resolution, `kiso instance *`, risoluzione implicita, connector port assignment su install
- [ ] 50c. `install.sh`: bot name + `~/.kiso/instances/{name}/` + `instances.json` + docker run con port range
- [ ] 50d. Docs: `docker.md` (riscrittura), `cli.md`, `config.md`, `connectors.md`, `logging.md` + `completions/`
