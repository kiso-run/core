...
- [x] `kiso/main.py`: sostituire il check startswith con `Path.is_relative_to()`
- [x] `tests/test_published.py`: aggiunto `test_path_traversal_sibling_dir_blocked` — directory con prefisso identico ma nome diverso viene rifiutata
...
**Fix**: sollevare `ValueError` invece di usare il fallback; `_report_pub_files()` cattura e logga, restituendo `[]`.

- [x] `kiso/pub.py`: rimosso fallback `or "kiso"`; sollevato `ValueError` con messaggio esplicito
- [x] `kiso/worker/utils.py`: `_report_pub_files()` cattura `ValueError` da `pub_token`, logga warning, restituisce `[]`
...
**Fix**: `grep -E '^KISO_LLM_API_KEY='` + `cut` per estrarre solo il valore necessario, senza eseguire il file.

- [x] `install.sh`: sostituito `source "$ENV_FILE"` con lettura sicura via grep/cut; variabile temporanea `_api_key_check` rimossa con `unset` dopo il controllo
- [x] `tests/bash/test_install_env_safe_read.bats`: 5 test — lettura normale, command injection bloccato, subshell bloccato, chiave assente, riga commentata ignorata
- [x] `docs/security.md`: sezione "Published File Security" riscritta (era errata: diceva UUID4, è HMAC-SHA256); aggiunto dettaglio su `is_relative_to`, symlink, token assente
- [x] `docs/api.md`: descrizione path traversal aggiornata con `is_relative_to` e vettori coperti; nota su CLI token assente
- [x] `tests/test_published.py`: aggiunti `test_path_traversal_symlink_blocked` e `test_path_traversal_absolute_filename_blocked`
- [x] `tests/test_worker.py`: aggiunto `test_empty_when_no_cli_token` per `_report_pub_files`
...
- [x] `kiso/store.py`: riscrivere `append_task_llm_call()` con `json_insert` nativo SQLite — usa `CASE WHEN json_valid(llm_calls) THEN llm_calls ELSE '[]' END` per gestire NULL e JSON corrotto
- [x] `tests/test_store.py`: `test_append_task_llm_call_atomic_no_data_loss` — 10 append concorrenti via `asyncio.gather`, verifica tutti presenti senza duplicati/perdite
...
**Analisi**: i due UPDATE sono già in un'unica transazione implicita aiosqlite (un solo `db.commit()` a fine funzione). Nessun bug reale.

- [x] `kiso/store.py`: verificato — nessuna modifica necessaria, i due UPDATE condividono già la stessa transazione implicita

### 60c. Processi zombie su `asyncio.TimeoutError` (`exec.py:35`, `skill.py:35`)

**Problema**: quando `asyncio.wait_for` scade, il processo figlio rimane in esecuzione. Nessun `proc.kill()` né `await proc.wait()` nel path di timeout.
...
- [x] `kiso/worker/exec.py`: aggiunto kill + wait nel handler `TimeoutError`
- [x] `kiso/worker/skill.py`: stessa fix
- [x] `tests/test_worker.py`: `test_timeout_kills_process` in `TestExecTask` e `TestSkillTask` — mock subprocess, verifica `kill` e `wait` chiamati su timeout
...
- [x] `cli/connector.py`: riscrivere `_write_status()` con pattern write-tmp + replace — usa `status_file.with_suffix(".tmp")` e `tmp_file.replace(status_file)`
- [x] `tests/test_cli_connector.py`: test esistenti verificano il comportamento; write atomico non altera l'interfaccia pubblica
...
### 61a. Connection pooling HTTP per chiamate LLM (`llm.py`)
...
**Fix**: client condiviso a livello di modulo, inizializzato da `main.py` lifespan. Fallback a client per-call quando `_http_client is None` (test, uso standalone).

- [x] `kiso/llm.py`: `_http_client: httpx.AsyncClient | None`, `init_http_client(timeout)`, `close_http_client()`. `call_llm` usa il client condiviso se disponibile, altrimenti `async with httpx.AsyncClient(...)`.
- [x] `kiso/main.py`: `init_http_client(timeout=exec_timeout)` all'avvio, `close_http_client()` allo shutdown.
- [x] `tests/test_llm.py`: 3 nuovi test (`TestSharedHttpClient`) — shared client usato, fallback per-call, init+close.

### 61b. Cache `discover_skills()` (`skills.py`)
...
**Fix**: `_skills_cache: list[dict] | None` a livello di modulo. Cache attiva solo con il path default (non quando si passa `skills_dir` custom). `invalidate_skills_cache()` chiamata dopo install/remove/update.

- [x] `kiso/skills.py`: `_skills_cache`, `invalidate_skills_cache()`, guard `use_cache = skills_dir is None`
- [x] `cli/skill.py`: `invalidate_skills_cache()` dopo install/remove/update (accanto a `invalidate_cache()` di sysenv)
- [x] `tests/test_skills.py`: `TestDiscoverSkillsCache` — 3 test (cache stessa istanza, custom dir bypassa cache, invalidate forza rescan)

### 61c. Audit dir init una sola volta (`audit.py`)

**Problema**: `mkdir(parents=True, exist_ok=True)` + `os.chmod(0o700)` eseguiti ad ogni scrittura nel log di audit.

**Fix**: `_audit_dir_ready: set[Path]` — la directory viene inizializzata solo al primo accesso per processo.

- [x] `kiso/audit.py`: `_ensure_audit_dir(path)` con guard `_audit_dir_ready`
- [x] `tests/test_audit.py`: `TestEnsureAuditDir` — 3 test (crea dir + permessi, mkdir chiamato una sola volta, N `_write_entry` → chmod una volta sola)

### 61d. `rglob` con `islice` per workspace grandi (`sysenv.py`)

**Analisi**: `_collect_workspace_files()` usa già un loop con `break` a `_MAX_SCAN=1000` candidati — non materializza mai l'intero tree. Il refactoring con `islice` sarebbe puramente cosmetico su logica già corretta e lazy.

- [x] `kiso/sysenv.py`: verificato — nessuna modifica necessaria

### 61e. `httpx.AsyncClient` singleton in `webhook.py`

**Problema**: il client viene creato dentro il loop di retry — 3 tentativi = 3 connessioni TCP.

**Fix**: `async with httpx.AsyncClient(...)` wrappa l'intero loop di retry.

- [x] `kiso/webhook.py`: `async with httpx.AsyncClient(timeout=10.0, follow_redirects=False) as client:` spostato fuori dal `for attempt, delay in ...`

### 61f. `rich.Console` singleton in `render.py`

**Analisi**: il `Console` in `render.py` è parametrizzato per-chiamata con `file=buf` (StringIO/AsciiBuffer), `width=caps.width`, `force_terminal`, `no_color` — tutti dipendenti dai `TermCaps` correnti. Non può essere singleton. Nessuna modifica.

- [x] `cli/render.py`: verificato — nessuna modifica necessaria
...
- [x] `kiso/worker/loop.py`: estratti `_handle_exec_task`, `_handle_msg_task`, `_handle_skill_task`, `_handle_search_task`, `_handle_replan_task` + `_PlanCtx` + `_TaskHandlerResult`
- [x] `kiso/worker/loop.py`: if/elif sostituito con `_TASK_HANDLERS.get(task_type)`. `_execute_plan` ridotto da ~533 a ~80 righe
- [x] `tests/test_worker.py`: `TestTaskHandlers` (9 test per handler in isolamento)

### 62b. Deduplicare il pattern task-step ✅

- [x] `kiso/worker/loop.py`: estratti `_store_step_usage()` e `_run_review_step()` — usati dagli handler exec, skill, search per eliminare duplicazione
- Nota: `_run_task_step` generale non estratto (pattern review/usage non abbastanza uniforme tra i tipi); estratti i due helper specifici che coprono ~80% della duplicazione effettiva

### 62c. Split `_process_message()` ✅

- [x] `kiso/worker/loop.py`: estratto `_run_planning_loop()` — while-True replan loop (~280 righe). `_process_message` ridotto da ~435 a ~105 righe
- [x] `kiso/worker/__init__.py`: esportati tutti i nuovi nomi per testabilità
- [x] `tests/test_worker.py`: `TestRunPlanningLoop` (2 test: success path, failure path)
...
- [x] `cli/plugin_ops.py`: aggiungere `_plugin_install()` parametrizzato (con `post_install` callback)
- [x] `cli/skill.py`: delegare a `_plugin_install`; `_skill_post_install()` per env vars, usage_guide, git exclude
- [x] `cli/connector.py`: delegare a `_plugin_install`; `_connector_post_install()` per env vars + config.example.toml
- [x] Test: comportamento verificato (patch target `check_deps_fn` passato come parametro)
...
- [x] Estratto in `kiso/plugins.py`: `_validate_plugin_manifest_base(manifest, plugin_dir, plugin_type)`
- [x] `kiso/skills.py`: `_validate_manifest` chiama base + check skill-specifici
- [x] `kiso/connectors.py`: `_validate_connector_manifest` delega direttamente a base
- [x] `tests/test_skills.py`: `TestValidatePluginManifestBase` (7 test)
...
- [x] `kiso/worker/utils.py`: aggiungere `_run_subprocess()` (shell/exec, stdin, uid, max_output_size)
- [x] `kiso/worker/exec.py`: delegare a `_run_subprocess`
- [x] `kiso/worker/skill.py`: delegare a `_run_subprocess`
- [x] `kiso/worker/__init__.py`: esportato `_run_subprocess`
- [x] `tests/test_worker.py`: `TestRunSubprocess` (5 test); patch target aggiornati a `kiso.worker.utils.asyncio.*`
...
- [x] `tests/test_cli.py`: `TestShouldStopPolling` (9 test), `TestRenderPlanStatus` (7 test)
...
- [x] `kiso/brain.py`: rimosso `validate_kwargs`; `validate_fn` ha sempre signature `(result) -> list[str]`; callers usano lambda dove necessario (`run_planner`, `run_curator`)
...
- [x] `cli/__init__.py`: `_ClientContext(NamedTuple)` + `_setup_client_context(args)` — usato da `_chat` e `_msg_cmd`
...
- [x] `kiso/store.py`: aggiungere `SessionDict`, `MessageDict`, `PlanDict`, `TaskDict`, `FactDict`; aggiornare signature delle funzioni store principali
...
- [x] `kiso/brain.py`: aggiungere `_prompt_cache: dict[str, str] = {}` e `invalidate_prompt_cache()`; fixture autouse in `test_brain.py` per isolamento
...
- [x] `tests/test_stats.py`: `TestFindPriceOrdering` — 8 test su gemini-2.x vs gemini-flash, gpt-4o-mini vs gpt-4o, llama-3.3 vs llama-3, ordine chiavi dict

### 65d. `build_secret_variants()` silenzioso su eccezione (`security.py:120`)

**Problema**: se la costruzione delle varianti di un segreto fallisce (es. encoding error), l'eccezione viene ingoiata silenziosamente.

- [x] `kiso/security.py`: aggiunto `import logging` + `log`; `logger.error(...)` nel blocco except
...
- [x] `kiso-host.sh`: già aveva `set -euo pipefail` ✓
- [x] `install.sh`: `base_url=""` inizializzato prima del blocco NEED_CONFIG; `tr -s '-'` in `_derive_instance_name`; `[[ ! -t 0 ]]` aggiunto a rilevamento non-interattivo in `ask_models`
- [x] `tests/bash/test_install.bats`: 10 test bats per `_derive_instance_name`

### 65f. Log rotation per session log (`log.py`)

**Problema**: i session log non vengono mai ruotati.

- [x] `kiso/log.py`: `RotatingFileHandler` per session log (2 MB, 2 backup); test `test_session_log_uses_rotating_handler`

### 65g. `SessionLogger` — aggiungere metodi `debug()` e `exception()` (`log.py:40`)

- [x] `kiso/log.py`: aggiunti `debug()` ed `exception()` a `SessionLogger`; test in `test_log.py`
...
- [x] `kiso/main.py`: estratto `_init_app_state()`
- [x] `tests/conftest.py`: aggiornata fixture `client` a usarla

---

## Milestone 66: Code review — performance, correttezza, robustezza

Miglioramenti individuati durante una code review sistematica del codebase. Nessun cambio di comportamento visibile all'utente.

---

### 66a. Performance — `_post_plan_knowledge` parallelismo

**Problema**: Curator (LLM call) e Summarizer (LLM call) eseguono serialmente anche se sono completamente indipendenti. Con entrambi attivi, il tempo post-plan include due chiamate LLM in sequenza. Decay e Archive (solo SQL) idem.

**Fix**: tre fasi invece di cinque sequenziali:
1. `asyncio.gather(curator, summarizer)` — indipendenti, nessuna dipendenza dati
2. Fact consolidation — dopo curator (vede i fatti appena promossi)
3. `asyncio.gather(decay, archive)` — solo SQL, indipendenti

- [x] `kiso/worker/loop.py`: `_post_plan_knowledge()` — fase 1: `asyncio.gather` su curator e summarizer; fase 3: `asyncio.gather` su decay e archive
- [x] `tests/test_worker.py`: `TestPostPlanKnowledgeParallel` — verifica che curator e summarizer vengano chiamati con `gather`; consolidation lanciata dopo; decay e archive con `gather`

---

### 66b. Correttezza — `update_task_review` non aggiorna `updated_at`

**Problema**: `update_task` e `update_task_substatus` aggiornano `updated_at = CURRENT_TIMESTAMP`; `update_task_review` no. Un task che riceve il verdetto del reviewer non aggiorna il timestamp — il polling `/status` può servire dati stale.

**Fix**: aggiungere `updated_at = CURRENT_TIMESTAMP` all'UPDATE.

- [x] `kiso/store.py`: `update_task_review()` — aggiunto `updated_at = CURRENT_TIMESTAMP` nella query UPDATE
- [x] `tests/test_store.py`: `test_update_task_review_bumps_updated_at` — verifica che `updated_at` cambi dopo la chiamata

---

### 66c. Robustezza — `PRAGMA busy_timeout` mancante

**Problema**: `init_db` imposta WAL ma non `busy_timeout`. Se due coroutine fanno commit ravvicinati (substatus update + task update nello stesso ciclo), SQLite può restituire `SQLITE_BUSY` immediatamente invece di attendere brevemente.

**Fix**: aggiungere `PRAGMA busy_timeout = 5000` (5 secondi) subito dopo `PRAGMA journal_mode=WAL`.

- [x] `kiso/store.py`: `init_db()` — `await db.execute("PRAGMA busy_timeout = 5000")`
- [x] `tests/test_store.py`: `test_busy_timeout_is_set` — verifica che il pragma sia attivo sul db di test

---

### 66d. API — `collect_deploy_secrets` parametro non usato

**Problema**: `collect_deploy_secrets(config=None)` dichiara `config` ma non lo legge mai. Ogni chiamante lo invoca senza argomenti. Il parametro suggerisce erroneamente che `config` influenzi la raccolta dei segreti.

**Fix**: rimuovere il parametro.

- [x] `kiso/security.py`: `collect_deploy_secrets()` — rimosso `config=None`
- [x] `kiso/worker/loop.py`: siti di chiamata invariati (già privi di argomento)
- [x] `tests/test_security.py`: verificare che i test esistenti passino

---

### 66e. Mantenibilità — `build_*_messages` dichiarate `async` senza `await`

**Problema**: `build_reviewer_messages`, `build_paraphraser_messages`, `build_summarizer_messages`, `build_searcher_messages`, `build_exec_translator_messages`, `build_messenger_messages`, `build_classifier_messages` sono `async def` ma non contengono nessun `await`. I chiamanti attendono una coroutine che non fa I/O, con overhead di scheduling inutile e un'API fuorviante sul contratto della funzione.

**Fix**: convertire a `def` regolari; aggiornare i siti di chiamata che le `await`.

- [x] `kiso/brain.py`: 7 funzioni `build_*_messages` da `async def` a `def`; rimossi gli `await` ai siti di chiamata interni a `brain.py`
- [x] `tests/test_brain.py`: verificare che i test esistenti passino (nessuna modifica funzionale)

---

### 66f. Robustezza — FTS5 fallback silenzioso

**Problema**: il blocco `except Exception` in `search_facts` che attiva il fallback a `get_facts` non logga l'eccezione. Se FTS5 fallisce per una causa inattesa (tabella corrotta, bug), il degradamento avviene invisibilmente.

**Fix**: aggiungere `log.debug(...)` nel blocco except con l'eccezione e `exc_info=True`.

- [x] `kiso/store.py`: `search_facts()` — `except Exception as exc: log.debug("FTS search failed, falling back: %s", exc, exc_info=True)`
- [x] `tests/test_store.py`: `test_fts5_fallback_logs_debug` — mock db che lancia eccezione sulla query FTS; verifica che `log.debug` venga chiamato

---

### 66g. Stile — `import json` dentro funzione + `encoding` mancante

**Problema 1**: `_strip_llm_verbose` in `main.py` importa `json` nel corpo della funzione (`import json as _json`). Ogni chiamata a `/status` esegue un import statement.

**Problema 2**: `_load_env_file` usa `path.read_text()` senza `encoding="utf-8"`. Tutto il resto del codebase specifica l'encoding esplicitamente (es. `_init_kiso_dirs:69`).

**Fix**: `import json` a livello di modulo; `encoding="utf-8"` in `read_text`.

- [x] `kiso/main.py`: `import json` spostato a livello modulo; `path.read_text(encoding="utf-8")` in `_load_env_file`

---

### 66h. Performance — `check_deps` rilegge `kiso.toml`

**Problema**: `discover_skills()` parsa `kiso.toml` per ogni skill e costruisce il dict. `check_deps()` riceve quel dict ma riapre e riparsa lo stesso `kiso.toml` per leggere `[kiso.deps]` — doppio I/O per la stessa informazione.

**Fix**: includere `"deps"` nel dict restituito da `discover_skills`; `check_deps` usa `skill.get("deps", {})` senza I/O.

- [x] `kiso/skills.py`: `discover_skills()` — aggiunto `"deps": manifest.get("kiso", {}).get("deps", {})` nel dict skill
- [x] `kiso/skills.py`: `check_deps()` — usa `skill.get("deps", {}).get("bin", [])` senza aprire file; mantiene fallback a lettura file se `"deps"` assente (compat con dict vecchi)
- [x] `tests/test_skills.py`: `test_check_deps_uses_skill_dict_no_io` — verifica che `open` non venga chiamato quando il dict contiene già `"deps"`

---

### 66i. Tipo e stile — `_PlanCtx` generics + import lungo

**Problema 1**: `_PlanCtx.deploy_secrets: dict`, `session_secrets: dict`, `installed_skills: list` usano tipi senza parametri. Inconsistente col resto del codebase.

**Problema 2**: `from kiso.llm import ...` in `loop.py` è una riga da 142 caratteri.

**Fix**: aggiungere parametri di tipo; wrappare l'import su più righe.

- [x] `kiso/worker/loop.py`: `_PlanCtx` — `deploy_secrets: dict[str, str]`, `session_secrets: dict[str, str]`, `installed_skills: list[dict]`
- [x] `kiso/worker/loop.py`: import `kiso.llm` wrappato con parentesi su più righe
