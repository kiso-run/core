...
def _print_post_init(config_path, preset_name: str) -> None:
    ...
    if preset_name == "default":
        print()
        print("Optional system dependencies (for local-first defaults):")
        print("  tesseract-ocr (+ ita/eng language packs)  — ocr-mcp v0.2 default")
        print("  whisper-cli + ggml model file             — transcriber-mcp v0.2 default")
        print()
        print("To opt back into v0.1 cloud-only behaviour without")
        print("installing the system deps:")
        print("  export KISO_OCR_BACKEND=gemini")
        print("  export KISO_TRANSCRIBER_BACKEND=gemini")
        print("  export KISO_SEARCH_MODEL_FAST=perplexity/sonar-pro")
    print()
    print("Next steps:")
