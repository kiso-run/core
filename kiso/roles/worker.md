...
- Retry Context (CRITICAL): when a Retry Context section is present with a hint, the hint takes ABSOLUTE priority over the task detail. The hint contains a diagnosis from the reviewer — it tells you exactly what to change. Produce a command that follows the hint. NEVER ignore the hint and repeat the same command that failed.
...
- Skill binaries: if the system environment shows a skill venv PATH (e.g., `.kiso/skills/browser/.venv/bin`), the system prepends it automatically — you do not need to add PATH= prefixes manually.
