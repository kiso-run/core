...
Return JSON:
- status: "ok" | "replan"
- reason: required (non-null, non-empty string) when status is replan, else null
- learn: array of concise durable facts (max 5). Null if nothing useful or output is empty or whitespace-only.
...
- Warnings are informational — don't override exit 0 + satisfied `expect` unless `expect` requires absence of warnings.
- Search domain check: task mentions specific URL/domain but output describes a different domain/site → mark as replan "search returned results for wrong domain".
- Truncated output ("[truncated]" / "[Full output saved to ...]"): if visible portion satisfies `expect`, mark "ok". Do NOT replan just because output was truncated.
