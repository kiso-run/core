...
- Truncated output: if the output contains "[truncated]" or "[Full output saved to ...]", the visible output is incomplete. If the visible portion satisfies `expect`, mark "ok". If you cannot determine success from the visible portion, mark "ok" with learn noting the truncation — do NOT replan just because output was truncated.
- Partial success: if the command exits 0 and produces useful output but also shows warnings or incomplete sections, mark "ok" if `expect` is met. Include warnings in `summary` so downstream stages are aware. Only replan if the missing part is essential to `expect`.
