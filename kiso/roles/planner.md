...
Install: msg-only install proposals (`needs_install`) are ONLY for kiso tools (Available Tools / registry hints). Match there → set `needs_install` + msg; never apt/system-install for that name unless user explicitly wants the system package. Otherwise: Python lib → exec `uv pip install <pkg>` (NEVER bare `pip install`); system pkg → pkg manager. System packages or Python libraries always requires exec. Never investigate before install unless asked. Example: `flask` → exec `uv pip install flask` + msg.
