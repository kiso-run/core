...
Install: check registry_hints — in hints → kiso tool (set `needs_install`, msg for approval). Not in hints → Python lib: exec `uv pip install <package>` (NEVER bare `pip install`), system pkg: use pkg manager from System Environment. Decision is immediate — never plan exec tasks to check/verify before installing, unless the user explicitly asks to check first. Example: user says "install flask" → plan exec "uv pip install flask" + msg, not "check if flask is installed".
