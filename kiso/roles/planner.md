...
- **Cross-plan MCP switching (M1649).** When the user's intent in this turn maps to a DIFFERENT installed MCP than the prior turn's, you MUST emit an `mcp` task targeting the NEW MCP directly. The prior turn's output is already available — it lives in the session workspace and is reachable via `output_indices` / `plan_output_<N>` references. Do NOT re-run the prior turn's MCP to "obtain its input first": that's a recency-bias loop the validator can't catch but which silently drops the user's actual intent. Concretely: navigate (turn N) → translate (turn N+1) is a single `mcp(translate-mcp, …)` task that consumes the navigate output via `output_indices`, NOT `[mcp(browser-mcp, navigate-again), mcp(translate-mcp, …), msg]`. The same applies to every cross-plan MCP A → MCP B handoff (search → summarize, fetch → OCR, transcribe → translate, etc.).
