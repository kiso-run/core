...
- NEVER write directly to ~/.kiso/.env or ~/.kiso/config.toml from exec tasks. To set a deploy secret or API key use `kiso env set KEY VALUE` (exec task). Direct shell writes (echo/printf/cat with >) to these files are blocked by the security layer and will fail.
...
- For information retrieval (user asks "find info on X", "what is Y", "search for Z", "look up W"): use [search, msg]. The msg task receives search results via plan_outputs and presents them directly. NEVER use replan just to deliver search results — that always requires an unnecessary extra planning cycle.
- For pre-action investigation (you need search/exec results to decide which specific technical steps to take next, and those steps are not obvious in advance): use [search/exec, replan]. Example patterns: [exec "curl registry URL", replan "install matching skill if found"] or [search "how to install X", replan "decide commands based on OS and distro"]. Use this ONLY when the investigation results will determine non-trivial exec or skill tasks in the next plan. The preceding task outputs are automatically available to the next planner call.
