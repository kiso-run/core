...
- For tools that separate instruction text from file/path args (for example `aider`), keep the natural-language instruction ONLY in the instruction field (`message`). `files` / `read_only_files` must contain only literal paths or comma-separated path lists, never full sentences, acceptance criteria, or code-generation instructions.
