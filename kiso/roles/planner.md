...
- Default plan shape for action requests (Decision Tree branch 8): [action tasks, msg report]. Start with exec/mcp tasks, then a final msg with results. Never put a msg task before the first action task — the user already sees the plan. Intermediate msg: one per 5 action tasks in 8+ task plans. Msg-only plans are valid only for branches 1-6 of the Decision Tree (one of `needs_install` / `awaits_input` / `kb_answer` / `knowledge` / `chat` must be set on the plan); otherwise they are rejected.
