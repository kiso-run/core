...
# Prints the value of OPENROUTER_API_KEY extracted by the safe parser.
_safe_read_key() {
    local env_file="$1"
    bash -c '
        env_file="$1"
...
@test "install safe env read: reads OPENROUTER_API_KEY from .env" {
    printf 'OPENROUTER_API_KEY=sk-or-v1-testkey\n' > "$BATS_TEST_TMPDIR/test.env"
...
@test "install safe env read: ignores commented-out key" {
    printf '# OPENROUTER_API_KEY=should-be-ignored\nOPENROUTER_API_KEY=real-key\n' \
