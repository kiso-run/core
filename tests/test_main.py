...
class TestInitKisoDirs:
    ...
    def test_syncs_reference_docs(self, tmp_path):
        ...
        assert (ref_dir / "wrappers.md").is_file()
        assert (ref_dir / "connectors.md").is_file()
        # Verify content is non-empty
        assert len((ref_dir / "wrappers.md").read_text()) > 0
    ...
    def test_only_writes_when_changed(self, tmp_path):
        ...
        with patch("kiso.main.KISO_DIR", tmp_path):
            _init_kiso_dirs()
        mtime1 = (tmp_path / "reference" / "wrappers.md").stat().st_mtime
        with patch("kiso.main.KISO_DIR", tmp_path):
            _init_kiso_dirs()
        mtime2 = (tmp_path / "reference" / "wrappers.md").stat().st_mtime
