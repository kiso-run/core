"""Tests for the dreamer (periodic knowledge consolidation)."""
...
from kiso.worker.loop import _maybe_run_dream, _LAST_DREAM_KV_KEY
...
def _make_config(overrides: dict | None = None):
    ...
    class FakeConfig:
        def __init__(self):
            ...
            self.models = {"dreamer": "test-dreamer"}
    return FakeConfig()
...
class TestValidateDreamer:
    ...
    def test_valid_result(self):
        ...
        assert validate_dreamer(result, ids) == []

    def test_missing_ids(self):
        ids = {1, 2, 3}
        result = {"delete": [1], "update": [], "keep": [2]}
        errors = validate_dreamer(result, ids)
...
class TestBuildDreamerMessages:
    ...
    def test_groups_by_entity(self):
        ...
        msgs = build_dreamer_messages(facts_by_entity)
        assert len(msgs) == 2
...
# ---------------------------------------------------------------------------
# _maybe_run_dream — gate logic
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
class TestMaybeRunDream:

    async def test_skips_when_too_recent(self, db):
        """Dream should not run if last dream was within interval."""
        config = _make_config({"dream_interval_hours": 24, "dream_min_facts": 1})
        # Set last dream to now
        await set_kv(db, _LAST_DREAM_KV_KEY, str(time.time()))
        # Add enough facts
        for i in range(5):
            await save_fact(db, f"fact number {i} for testing", "test", category="general")

        with patch("kiso.worker.loop.run_dreamer", new_callable=AsyncMock) as mock_dreamer:
            await _maybe_run_dream(db, config, "test-session", 5)
            mock_dreamer.assert_not_called()

    async def test_skips_when_too_few_facts(self, db):
        """Dream should not run if fewer than dream_min_facts."""
        config = _make_config({"dream_interval_hours": 0, "dream_min_facts": 100})
        # No last dream time, but too few facts
    ...
    async def test_runs_when_gates_pass(self, db):
        """Dream should run when enough time has passed and enough facts exist."""
        config = _make_config({"dream_interval_hours": 1, "dream_min_facts": 2})
        # Last dream was long ago
        await set_kv(db, _LAST_DREAM_KV_KEY, str(time.time() - 7200))
        # Enough facts
        for i in range(3):
            await save_fact(db, f"fact number {i} is long enough", "test", category="general")

        dream_result = {"delete": [], "update": [], "keep": []}
        with patch("kiso.worker.loop.run_dreamer", new_callable=AsyncMock, return_value=dream_result) as mock_dreamer:
            with patch("kiso.worker.loop.apply_dream_result", new_callable=AsyncMock) as mock_apply:
                await _maybe_run_dream(db, config, "test-session", 5)
                mock_dreamer.assert_called_once()
                mock_apply.assert_called_once_with(db, dream_result)

    async def test_runs_when_never_dreamed(self, db):
        """Dream should run when no last_dream_time exists."""
        config = _make_config({"dream_interval_hours": 1, "dream_min_facts": 2})
        for i in range(3):
            await save_fact(db, f"fact number {i} is long enough", "test", category="general")

        dream_result = {"delete": [], "update": [], "keep": []}
        with patch("kiso.worker.loop.run_dreamer", new_callable=AsyncMock, return_value=dream_result):
            with patch("kiso.worker.loop.apply_dream_result", new_callable=AsyncMock):
                await _maybe_run_dream(db, config, "test-session", 5)
                # Should have set the timestamp
                raw = await get_kv(db, _LAST_DREAM_KV_KEY)
                assert raw is not None
                assert float(raw) > 0

    async def test_handles_dreamer_error(self, db):
        """DreamerError should be caught, not propagated."""
        config = _make_config({"dream_interval_hours": 1, "dream_min_facts": 1})
        await save_fact(db, "fact for dreamer error test", "test", category="general")

        with patch("kiso.worker.loop.run_dreamer", new_callable=AsyncMock, side_effect=DreamerError("bad")):
            # Should not raise
            await _maybe_run_dream(db, config, "test-session", 5)
    ...
    async def test_handles_timeout(self, db):
        ...
        async def slow_dreamer(*a, **kw):
            await asyncio.sleep(100)

        with patch("kiso.worker.loop.run_dreamer", new_callable=AsyncMock, side_effect=slow_dreamer):
            # timeout=0.01 should trigger TimeoutError quickly
            await _maybe_run_dream(db, config, "test-session", 0)
