...
class TestRunWorker:
    ...
    async def test_plan_with_secrets_extracts_them(self, db, tmp_path):
        """Secrets from the plan are extracted and available for wrapper execution."""
        config = make_config()
        await create_session(db, "sess1")
...
class TestBuildReplanContext:
        ...
        history = [
            {
                ...
                "failure": "wrapper not found",
                "what_was_tried": ["[exec] kiso wrapper install browser"],
    ...
    def test_confirmed_facts_from_registry_list(self):
        """extract wrapper names from registry JSON array."""
        import json as _json
...
class TestExecutePlan:
    ...
    async def test_replan_task_triggers_replan(self, db, tmp_path):
        ...
        await create_task(db, plan_id, "sess1", type="replan",
                          detail="install appropriate wrapper")
        ...
        assert "install appropriate wrapper" in reason
...
class TestFormatPlanOutputsForMsg:
    ...
    def test_mixed_with_and_without_summary(self):
        ...
        outputs = [
            ...
            {
                "index": 2, "type": "exec", "detail": "fetch data",
    ...
    def test_oversized_raw_falls_back_to_reviewer_summary(self):
        ...
        outputs = [
            {
                "index": 1, "type": "exec", "detail": "fetch HTML",
...
class TestSelfDirectedReplan:
    ...
    async def test_self_directed_replan_marks_plan_done(self, db, tmp_path):
        ...
        config = make_config()
        await create_session(db, "sess1")
        msg_id = await save_message(db, "sess1", "alice", "user", "install search wrapper", processed=False)

        investigation_plan = {
            "goal": "Investigate available wrappers",
            "secrets": None,
            "extend_replan": None,
            "tasks": [
        ...
        queue: asyncio.Queue = asyncio.Queue()
        await queue.put({"id": msg_id, "content": "install search wrapper", "user_role": "admin"})
...
class TestExtendReplan:
    ...
    async def test_extend_replan_cumulative_cap(self, db, tmp_path):
        ...
        async def _reviewer(*a, **kw):
            idx = min(review_idx[0], len(_distinct_reasons) - 1)
            review_idx[0] += 1
            return {"status": "replan", "reason": _distinct_reasons[idx],
                    "learn": None, "retry_hint": None, "summary": None}
...
class TestAutoPublishToolFiles:
    ...
    def test_snapshot_and_publish(self, tmp_path):
        ...
        with _patch_kiso_dir(tmp_path):
            ...
            # Simulate wrapper creating a file
...
class TestDetectCircularReplanUnit:
    ...
    def test_word_overlap_detected(self):
        from kiso.worker.loop import _detect_circular_replan
        history = [
            {"failure": "browser wrapper not installed cannot navigate to site", "goal": "g"},
            {"failure": "browser wrapper not installed cannot navigate to the site", "goal": "g"},
        ]
    ...
    def test_word_overlap_at_50pct_detected(self):
        ...
        from kiso.worker.loop import _detect_circular_replan
        # 5/10 words match = exactly 50%: {wrapper, not, found, in, the}
        history = [
            {"failure": "wrapper not found in the registry listing output check", "goal": "g"},
            {"failure": "wrapper not found in the system after installation attempt", "goal": "g"},
        ]
        assert _detect_circular_replan(history, history[-1]["failure"]) is True
    ...
    def test_goal_install_without_fail_keywords_not_stuck(self):
        ...
        from kiso.worker.loop import _detect_circular_replan
        history = [
            {"failure": "timeout waiting for response", "goal": "install websearch wrapper"},
...
class TestCircularReplanDetection:
    ...
    async def test_stuck_message_on_similar_failures(self, db, tmp_path):
        ...
        review_reasons = [
            "browser wrapper not installed, cannot navigate to site",
            "browser wrapper not installed, cannot navigate to the site",
            "browser wrapper still not installed, cannot navigate",
        ]
        review_idx = [0]
...
class TestProcessMessagePhaseCallback:
    ...
    async def test_phase_transitions_for_chat(self, db, tmp_path):
        """M1620: every message goes briefer → planner → execute. Phases
        should be planning → executing → idle (the classifying phase is
        retired with the classifier itself)."""
        conn, msg_id = db
        config = make_config()
        ...
        chat_plan = {
            "goal": "Reply",
            "secrets": None,
            "tasks": [{"type": "msg", "detail": "Say hi", "args": None, "expect": None}],
        }
        ...
        with patch("kiso.worker.loop.run_planner", side_effect=_planner), \
             patch("kiso.worker.loop.run_messenger", new_callable=AsyncMock, return_value="Hi!"), \
             patch("kiso.worker.loop._post_plan_knowledge", new_callable=AsyncMock), \
    ...
    async def test_replan_emits_planning_then_executing_phases(self, db, tmp_path):
        ...
        # M1620: every message starts with planning (briefer + planner) →
        # executing → (replan) planning → executing → idle
...
class TestSuggestedFixesSection:
    ...
    def test_suggested_fixes_from_history(self):
        ...
        history = [{
            ...
            "what_was_tried": ["[wrapper] browser navigate"],
            "key_outputs": [],
...
class TestE2EWebScenario:
    ...
    state (no wrappers). Verifies the system correctly:
    1. Uses search for content understanding
    2. Checks registry when screenshot needed
    3. Installs browser wrapper
    4. Uses browser wrapper for screenshot
    ...
    async def test_multi_plan_registry_then_install_then_use(self, db, tmp_path):
        """Full flow: plan1 (search + registry check) → plan2 (install) → plan3 (use wrapper + msg)."""
        config = make_config(settings={
            "worker_idle_timeout": 1,
            "llm_timeout": 5,
            "max_validation_retries": 1,
        ...
        # Plan 1: fetch + registry check + replan (was search + registry check + replan
        # before the v0.10 search retirement — kept structurally identical, just swapped
        # the first step from type=search to type=exec so the multi-plan flow remains
        # a faithful regression guard for the replan chaining under test).
        plan1 = {
...
class TestMsgTaskBrieferIntegration:
    ...
    def _plan_outputs(self):
        return [
            ...
            {"index": 2, "type": "exec", "detail": "curl -s gazzetta.it", "output": "news headlines here", "status": "done"},
...
class TestBroadenedCircularDetection:
    ...
    async def test_non_consecutive_word_overlap_detected(self, db, tmp_path):
        ...
        await create_session(db, "sess1")
        msg_id = await save_message(db, "sess1", "alice", "user", "fill form", processed=False)
        ...
        # Entry[0]: "button not found on page"
        # Entry[1]: "completely different unrelated error message"  (different)
        # Entry[2]: "button not found on form"  (>60% overlap with entry[0])
        review_reasons = [
            "button not found on page",
            "completely different unrelated error message",
            "button not found on form",
        ]
        review_idx = [0]
        ...
        queue: asyncio.Queue = asyncio.Queue()
        await queue.put({"id": msg_id, "content": "fill form", "user_role": "admin"})
        ...
        # After entry[2], broadened detection should find overlap with entry[0]
        stuck_msgs = [m for m in saved_messages if "trouble" in m.lower() or "stuck" in m.lower()]
        assert len(stuck_msgs) >= 1, (
            f"Broadened detection should catch non-consecutive overlap. Messages: {saved_messages}"
        )
...
class TestBlockExtendWhenStuck:
    ...
    async def test_extend_denied_when_stuck(self, db, tmp_path):
        ...
        await create_session(db, "sess1")
        msg_id = await save_message(db, "sess1", "alice", "user", "do task", processed=False)
        ...
        # Same failure reason → stuck detected from replan 2 onward
        async def _reviewer(*a, **kw):
            return {"status": "replan", "reason": "button not found on page"}

        queue: asyncio.Queue = asyncio.Queue()
        await queue.put({"id": msg_id, "content": "do task", "user_role": "admin"})
...
class TestFailTaskAndAudit:
    ...
    async def test_returns_stop_result(self, ctx):
        ...
        with patch("kiso.worker.loop.update_task", new_callable=AsyncMock) as mock_update, \
            ...
            result = await _fail_task_and_audit(ctx, 42, "wrapper", "do stuff", "broken", 3)
        assert result.stop is True
        assert result.stop_success is False
...
_FORMAT_TASK_LIST_CASES = [
    ...
    (
        [{"type": "exec", "detail": "find X"}],
        "Remaining",
        ["Remaining (1):", "- [exec] find X"],
    ),
]
...
class TestBuildExecutionBatches:
    ...
    def test_single_parallel_group(self):
        from kiso.worker.loop import _build_execution_batches
        tasks = [
            {"type": "exec", "detail": "A", "parallel_group": 1},
            {"type": "exec", "detail": "B", "parallel_group": 1},
            {"type": "msg", "detail": "C"},
        ]
        batches = _build_execution_batches(tasks)
    ...
    def test_preserves_task_indices(self):
        ...
        tasks = [
            {"type": "exec", "detail": "A", "parallel_group": 1},
            {"type": "exec", "detail": "B", "parallel_group": 1},
            {"type": "exec", "detail": "C", "parallel_group": 1},
            {"type": "msg", "detail": "D"},
        ]
        batches = _build_execution_batches(tasks)
    ...
    def test_null_group_treated_as_sequential(self):
        from kiso.worker.loop import _build_execution_batches
        tasks = [
            {"type": "exec", "detail": "A", "parallel_group": None},
            {"type": "exec", "detail": "B", "parallel_group": None},
        ]
        batches = _build_execution_batches(tasks)
...
class TestParallelGroupFromDB:
    ...
    async def test_db_tasks_with_parallel_group(self, tmp_path):
        ...
        assert "exec alpha" in details
        assert "exec beta" in details
        await db.close()
...
class TestWriteLastPlanSummary:
    ...
    def test_writes_summary_with_produced_files(self, tmp_path):
        ...
        with patch("kiso.worker.utils.KISO_DIR", tmp_path):
            ...
            pre = set(ws.rglob("*"))
            # Simulate a wrapper producing a file
            (ws / "screenshot.png").write_bytes(b"x" * 100)
            completed = [
                {"type": "exec", "wrapper": "browser", "reviewer_summary": "Screenshot taken successfully"},
            ]
    ...
    def test_provenance_omitted_when_no_plan_outputs(self, tmp_path):
        ...
        copy `wrapper`/`wrapper`/`index` from arbitrary task rows, fabricating
        ...
        with patch("kiso.worker.utils.KISO_DIR", tmp_path):
            ...
            completed = [
                {"type": "exec", "wrapper": "browser", "reviewer_summary": "did stuff"},
            ]
